...
- **[`app/src/app/api/v1/readings/route.ts`](app/src/app/api/v1/readings/route.ts)** is a Next.js Route Handler that server-side-joins `buoyState.all()` + `oceanReading.all()` into clean JSON — this is the actual shape of "the paid product." It's left open (no API key) for the demo; a real deployment would gate/meter it per the pricing tier, which the page's copy states explicitly rather than pretending it's already enforced.
